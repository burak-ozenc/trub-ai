 ...
 * NOW WITH PROPER REST RENDERING
 */
const SheetMusicViewer: React.FC<SheetMusicViewerProps> = ({
    ...
    // RESTORED: Original array-based refs
    ...
    const loadAndRenderMidi = async () => {
        ...
        try {
            ...
            if (header !== 'MThd') {
                throw new Error(`Invalid MIDI file. Header is "${header}"`);
            }

            const midi = new Midi(arrayBuffer);

            if (midi.tracks.length === 0 || !midi.tracks[0]?.notes?.length) {
                throw new Error('MIDI file has no notes');
            }

            // Convert with REST support
            const vexFlowData = convertMidiToVexFlow(midi);

            console.log('🎵 Converted to VexFlow:', {
                noteCount: vexFlowData.notes.length,
                eventCount: vexFlowData.events?.length || 0,
    ...
    /**
     * Color notes based on validation results - RESTORED with stem support
     */
    ...
    const updateVisualFeedback = () => {
        ...
        if (currentNoteResult && currentNoteResult.progress !== undefined) {
            ...
            progressSource = 'flow-mode';
        }

        // Debug logging (throttled)
        if (Math.random() < 0.1 || progress >= 0.8) {
            console.log('🎯 Visual feedback update:', {
                noteIndex: currentNoteIndex,
                progress: (progress * 100).toFixed(0) + '%',
                source: progressSource,
                currentNoteResult: currentNoteResult ? {
                    isCorrect: currentNoteResult.isCorrect,
                    progress: currentNoteResult.progress
                } : null,
                expectedNote: expectedNote ? expectedNote.pitch : null
            });
        }
    ...
    /**
     * Draw highlight box around current note - RESTORED
     */
    ...
    /**
     * Draw or update progress bar under current note - RESTORED enhanced version
     */
    const drawProgressBar = (noteIndex: number, progress: number) => {
        ...
        try {
            ...
            // Enhanced bar width - match note width or minimum 80px
            ...
            // Check if we already have a progress bar for this note
            const existingGroup = progressBarRef.current;

            if (existingGroup) {
                // Update existing progress bar (smoother than recreating)
                const progressRect = existingGroup.querySelector('rect:last-child');
                if (progressRect) {
                    ...
                    console.log('📊 Updated progress bar:', {
                        noteIndex,
                        progress: (progress * 100).toFixed(0) + '%',
                        barWidth: newWidth.toFixed(1) + 'px'
                    });
                    return;
                }
            }
            ...
            group.setAttribute('style', 'transition: all 0.1s ease-out');
            ...
            progressRect.setAttribute('style', 'transition: width 0.1s ease-out, fill 0.2s ease-out');
            ...
            console.log('📊 Created progress bar:', {
                noteIndex,
                progress: (progress * 100).toFixed(0) + '%',
                barWidth: barWidth.toFixed(1) + 'px',
                position: { x: barX.toFixed(1), y: barY.toFixed(1) }
            });
        } catch (err) {
